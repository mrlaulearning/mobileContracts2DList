...
def main():
    ...
    def fileRead():
        ...
        for customers in range(len(custList)):
            custList[customers][2]=datetime.strptime(custList[customers][2], "%d/%m/%y")
            custList[customers][3]=datetime.strptime(custList[customers][3], "%d/%m/%y")

        #sort the list by end date
        nameSort3=sorted(custList, key=itemgetter(3),reverse=False)

        #format the datetime objects back into strings
        for cust in range(len(nameSort3)):
            nameSort3[cust][2]=nameSort3[cust][2].strftime("%d/%m/%y")
            nameSort3[cust][3]=nameSort3[cust][3].strftime("%d/%m/%y")

        print("\nSorted by endDate (descending), formatted:")
        
        for contracts in range(len(nameSort3)):
            for fields in range(len(nameSort3[0])):
                print(str(custList[contracts][fields])+pad*(12-len(str(custList[contracts][fields]))),end="")
            print() 
